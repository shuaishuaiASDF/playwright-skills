...
- 如果任务先命中了 `$playwright-common`，但实际诉求是首次初始化 `ui-test/`，必须立即停止在 `playwright-common` 内继续展开，转用 `$ctj-playwright-bootstrap`。
- 如果当前仓库已经存在稳定的 `ui-test/` 基础框架，只剩局部配置补充、README 修订或业务脚本开发，不再触发本 skill，改走对应维护类 skill。
- 如果用户没有显式说“初始化”，但检查发现项目根本没有 `ui-test/package.json`、`ui-test/tsconfig.json`、`ui-test/playwright.config.ts` 这类基础文件，应先向用户说明“当前判断为首次基础框架缺失，建议改用 `$ctj-playwright-bootstrap`”。
...
- `code/test.config.ts`: 统一管理环境地址、角色账号、超时、选择器、数据库开关和报告开关。
- `code/.auth/`: 存放 Playwright 登录态和动态生成账号文件，必须加入忽略规则。
- `code/base/capabilities/auth/`: 登录、登录态 fixture、已登录测试入口、角色会话复用。
- `code/base/capabilities/common/`: 等待工具、接口结果封装、接口快照采集、审计记录基础类型。
- `code/base/capabilities/db/`: 数据库配置、运行时定位、查询执行封装。
- `code/base/capabilities/reporters/`: 审计报告生成器，至少支持 Markdown 和 JSON。
- `code/base/scripts/db/`: 数据库执行脚本或命令适配层。
- `code/case/`: 按业务模块存放 Playwright spec 和前置准备脚本。
- `code/e2e/`: 跨模块端到端组合脚本。
- `docs/`: 测试矩阵、用例说明、接口字段审计说明。
- `original/fixtures/`: 原始真实样本，按业务域继续分层。
- `reports/`: 审计报告、截图证据和运行过程产物，由 `AuditReporter` 运行时创建，不默认预创建。
- `playwright-report/`: Playwright HTML 报告输出，由 Playwright 运行时创建，不默认预创建。
- `test-results/`: Playwright 原始运行结果，由 Playwright 运行时创建，不默认预创建。

## M4 公共能力复制清单

初始化时只复制跨项目可复用能力。以下清单之外的认证、选择器、页面操作、业务模块能力不允许直接复制到新项目。

数据库能力：

- 复制 `ui-test/code/base/capabilities/db/db-config.ts` 到目标项目同名目录。
- 复制 `ui-test/code/base/capabilities/db/python-runtime.ts` 到目标项目同名目录。
- 复制 `ui-test/code/base/capabilities/db/query-executor.ts` 到目标项目同名目录。
- 复制 `ui-test/code/base/scripts/db/db-executor.py` 到目标项目同名目录。
- 复制 `ui-test/.env.db.local` 到目标项目同名目录，作为数据库本地配置初始文件。
- 复制后必须检查 `DB_EXECUTOR_PATH`，让它指向真实存在的 `db-executor.py`。
- SQL 执行能力只允许读库查询；禁止把写库、DDL、批量修复 SQL 放进通用执行入口。
- 数据库驱动依赖不写死到 Node 依赖中，README 必须说明 Python 侧按数据库类型安装 `pymysql`、`oracledb` 或 `dmPython`。
- `.env.db.local` 只写占位值和注释说明，不写真实数据库账号密码；初始化完成后由用户手工补齐真实连接信息。

报告能力：

- 复制 `ui-test/code/base/capabilities/reporters/audit-reporter.ts` 到目标项目同名目录。
- 复制后必须检查默认 `outputDir`，保证进入 `ui-test/` 执行时落到 `reports`。
- 保留 `AUDIT_RUN_ID`、章节分组、接口原证、DB 原证、截图路径、JSON 合并写回能力。
- 报告输出至少包含 Markdown 和 JSON，便于人工审计与后续汇总。
...
- 若当前仓库真实实现已修复缺陷，而模板未同步，先更新模板再用于新项目初始化。
- `references/initial-readme-template.md` 如仍存在，只作为历史参考；实际初始化以 `assets/templates/ui-test/README.md` 为准。
...
目录落地后，还要检查 Playwright 运行前置条件；不能只建目录不检查可运行性。

必查项：

- `ui-test/package.json` 是否存在以下开发依赖：
  - `@playwright/test`
  - `typescript`
  - `@types/node`
- `ui-test/package.json` 是否存在以下脚本：
  - `test`
  - `test:headed`
  - `test:list`
- `ui-test/tsconfig.json` 是否覆盖：
  - `playwright.config.ts`
  - `code/**/*.ts`
- `ui-test/playwright.config.ts` 是否至少包含：
  - `testDir`
  - `reporter`
  - `outputDir`
  - `use.baseURL`
  - `projects`
- `playwright.config.ts` 的 `testDir` 是否与实际目录一致，默认指向 `code/case`
- `reporter` 是否输出 `playwright-report`
- `outputDir` 是否输出 `test-results`
- `README.md` 是否已写明进入 `ui-test/` 执行 `npm install` 与 `npx playwright install chromium`
- 启用数据库能力时，`ui-test/.env.db.local` 是否已落地，且每个配置项是否写明用途
